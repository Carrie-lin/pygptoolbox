...
def dihedralAngleMat(V,F):
	'''
	DIHEFRALANGLEMAT computes the dihedral angle matrix between faces 
	...
	A = sparse.csr_matrix((signedDihedral, (adjFList[:,0], adjFList[:,1])), shape=(F.shape[0], F.shape[0]), dtype=np.float32)
