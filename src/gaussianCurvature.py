...
def gaussianCurvature(V,F):
    '''
    GAUSSIANCURVATURE  computes gaussian curvature of a triangle mesh
    ...
    Output:
    K: |V| numpy array of gaussian curvature at each vertex
    ...
    K = 2*np.pi - csr_matrix( (data, (rowIdx, colIdx)), shape=(V.shape[0],1), dtype = np.float32).todense()
    VA = vertexAreas(V,F)
