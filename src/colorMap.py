...
def colorMap(val, colormap = "default", valRange = None):
    if colormap == "blue":
        ...
        baseColor = np.array([[33,102,172],
            [67,147,195],[146,197,222],
            [209,229,240],[247,247,247],
            [253,219,199],[244,165,130],
            [214,96,77],[178,24,43]])
    x = np.copy(val)
    try: 
        if len(valRange) == 2:
            x -= valRange[0]
            x /= (2*valRange[1]+1e-16)
    except:
        x -= x.min()
        x /= (x.max()+1e-16)
